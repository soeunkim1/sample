...
var stdCgdCRec = function() {
	 ...
	 * @author 박갑수 at 2016. 4. 4
	 */
	moPage.onModelConstructDone_StdCgdCRec = function() {
		// 리피트 초기 설정
...
//				ExtControl.refreshBind("bindDivcls");
		...
		util.Submit.send(app, "subReTlsnChk", function(pbSuccess){
			if(pbSuccess){
				...
				if(vnCnt > 0){
					...
					moIdsForReTlsn.iObjDivRcd 	= vsObjDivRcd;
					moIdsForReTlsn.iKeyDate 		= vsEndDt;
					
					ExtPopUp.openLayeredPopup("/xtm/cgd/stdCgdPReTlsn.xtm", 700, 280);
	...
	moPage.callbackReTlsnPopup= function(paRtnValue){
		// [0]재수강학년도, [1]재수강학기, [2]재수강과목코드, [3]재수강과목명, [4]재수강과목명
		var vnIdx = util.Grid.getIndex(app, "grdCgdRec");
