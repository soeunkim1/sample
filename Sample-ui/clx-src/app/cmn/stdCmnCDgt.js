...
//	util.Udc.setUdcAppProperty("dipKeyDateHistory", "iUnitSystem", "CMN");
	...
	util.Submit.send(app, "subOnLoad", function(pbSuccess){
		if(pbSuccess){
			...
			util.SelectCtl.selectItem(app, "cbbDgDivRcd", 0);
			util.Control.redraw(app, ["dipKeyDate","cbbDgDivRcd","ipbLanDivRcdList"]);     
			//임베디드 되었을때 세팅
...
function doOnLoadWithAcademicStructure(){
	...
	if (voParam.mode) {
		...
		var voGrpActualRect = util.Control.getActualRect("grp_grdManage");
 ...
 * @return void
 */
function doSearch(arg, poCallbackFunc, psDgCd){
	// 1. 조회조건 필수체크 : 기준일자
	...
	util.Submit.send(app, "subListManage", function(pbSuccess){
		if(pbSuccess){
			...
			//학위코드가 넘어 오면 로우 포커싱 해줌
			if(ValueUtil.fixNull(vsfindDgCd) != "")
			{
//				util.Grid.findRowByCondition("grdManage", "DG_CD == " + vsfindDgCd);
				//이력탭 조회
...
//	{
//		ExtControl.setEnable(true,"ipbFrfDgCd");
//	}else
//	{
//		ExtControl.setEnable(false,"ipbFrfDgCd");
//	}
 ...
 * @desc   이력관리 (그리드) 내역 조회
 * @return void
 * @author hyunteak 2015. 11. 20.
 */
function doSearchHistory(poCallBackFunc){
	
	var vsHisDate = util.DataMap.getValue(app, "dmResOnLoad", "strKeyDateHistory");
	var vsKeyDate = util.DataMap.getValue(app, "dmResOnLoad", "strKeyDate");
	//폐기/복구일자 모두 지워졌을 경우 조회조건 기준일자 셋팅 한다.
	if(vsHisDate =="")
	{
		util.DataMap.setValue(app, "dmResOnLoad", "strKeyDateHistory", vsKeyDate);
	}
	...
	var vsLangKey = "";//언어키
	if(voSelectRowIdx != -1) {
		vsDgCd        = util.Grid.getCellValue(app, "grdManage", "DG_CD", voSelectRowIdx); //학위코드
	...
	//이력관리 조회 전 리퀘스트값 셋팅(학위코드 목록 포커싱된  Row 학위코드,언어코드)
	util.DataMap.setValue(app, "dmReqSelRow", "dgCd", vsDgCd);
	util.DataMap.setValue(app, "dmReqSelRow", "lanGbn", vsLangKey);
...
function doSearchLang(poCallBackFunc){
	...
	if (voSelRow != -1) {
		vsDgCd = util.Grid.getCellValue(app, "grdManage", "DG_CD", voSelRow);//학위코드
	...
	//언어 조회 전 리퀘스트값 셋팅(학위 목록 포커싱된  Row 학위코드,참조키)
	util.DataMap.setValue(app, "dmReqSelRow", "dgCd", vsDgCd);
...
function doInsert() {
	...
	util.Grid.setCellValue(app, "grdManage", "OBJ_DIV_RCD", "CC009DG", vnInsIdx);//객체구분코드
	util.Grid.setCellValue(app, "grdManage", "SP_OBJ_DIV_RCD", "CC009SP", vnInsIdx);//이수과정객체구분코드
	...
	// 프리폼의 학위코드에 포커스
	util.Control.setFocus(app, "ipbFrfDgCd");
	return vnInsIdx;
}
...
function doSave(paTabpageId, poCallbackFunc){
	...
	if (paTabpageId == TAB.LANG) {
		util.Submit.send(app, "subSaveLang", function(pbSuccess){
			if(pbSuccess) {
					...
					util.Tab.setSelectedTabItem(app, "tabMain", TAB.LANG);
				}
		});
	//데이터관리 프리폼   
		...
		util.Submit.send(app, "subSaveManage", function(pbSuccess){
			if(pbSuccess) {
					...
					util.Control.reset(app, "frfManage");
	...
	//데이터이력 	
		...
		util.Submit.send(app, "subSaveHistory", function(pbSuccess){
			if(pbSuccess) {
					flg ="Y";
					util.Grid.setFindRowCondition(app, "grdManage", "DG_CD == " + "'" + util.Grid.getCellValue(app, "grdManage", "DG_CD") + "'");
					doSaveWithAcademicStructure();
					util.Msg.notify(app, "NLS-INF-M001");
					//이력조회
					doSearchHistory(function(pbSuccess) {
						//저장후 기본정보 텝으로 이동
						var vsDgCd = util.DataMap.getValue(app, "dmReqSelRow", "dgCd");
						doSearch(null, poCallbackFunc,vsDgCd);
					});
				}else {
					util.Msg.notify(app, "NLS-WRN-M119");
					util.Tab.setSelectedTabItem(app, "tabMain", TAB.HIS);
...
//		}
	...
	var vsKeyDt = null;
	vsKeyDt = util.DataMap.getValue(app, "dmResOnLoad", "strKeyDateHistory")
...
function doRecoverHistory() {
		...
		if (vnMaxEndDt >= vsHistoryKeyDate) {
		 ...
		 var vsMsgCiiDate = vsHistoryKeyDate.substring(0, 4) + util.Msg.getMsg("NLS-SCR-YEAR") + vsHistoryKeyDate.substring(4, 6) 
						+ util.Msg.getMsg("NLS-SCR-MONTH") + vsHistoryKeyDate.substring(6, 8) + util.Msg.getMsg("NLS-SCR-DAY");
...
function onGrid_ids_btn2Insert(/* cpr.event.CUIEvent */ e){
	...
	//학위코드.참조키
	var vnRowLan = util.Grid.getIndex(app, "grdLang");
	var vsRefKey = util.DataMap.getValue(app, "dmReqSelRow", "refKey");
	var vsDgCd = util.DataMap.getValue(app, "dmReqSelRow", "dgCd");
	util.Grid.setCellValue(app, "grdLang","REF_KEY",vsRefKey,vnRowLan);
	util.Grid.setCellValue(app, "grdLang","DG_CD",vsDgCd,vnRowLan);
}
...
//util.Grid.copyToGridData(app, 소스그리드ID, 타겟그리드ID, 소스행인덱스, 타겟행인덱스)로 변경 필요...
	...
	util.Control.redraw(app, "grdManage");//이수과정목록 refresh
 ...
 * 사용자가 컨트롤을 클릭할 때 발생하는 이벤트.
 */
function onBtnDel2Click2(/* cpr.events.CMouseEvent */ e){
	/** 
	 * @type cpr.controls.Button
	 */
	var btnDel2 = e.control;
