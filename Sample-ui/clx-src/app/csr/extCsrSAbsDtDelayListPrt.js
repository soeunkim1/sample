...
///<xtmlink path="./extCsrSAbsDtDelayListPrt.xtm"/>

var extCsrSAbsDtDelayListPrt = function() {
...
//		ExtGroup.initSearchBox("grpSearch",["grp_rptCsrShreg"]);
		...
		util.Submit.send(app, "subOnLoad", function(pbSuccess){
			if(pbSuccess){
				...
				// 조회조건 초기화
				util.Control.redraw(app, ["ipbStDt", "ipbEndDt","cbbObjDivRcd"]);
	...
	function doList(poCallBackFunc) {
		...
		var vsSchTitle = "기간 : " + vsStDt + " ~ " + vsEndDt;
		...
		util.Submit.send(app, "subList",  function(pbSuccess) {
			if(pbSuccess){
				...
				var voOzFormParam = {
						  TITLE :  "기 일 연 장 자 명 단"//리포트타이틀
						, SUB_TITLE : "" //리포트서브타이틀		
						, FORM_TYPE : "flash"
					};	
						
				util.Report.open(app, "hojReport", "extCsrSAbsDtDelayListPrt", voOzFormParam, voParam);
