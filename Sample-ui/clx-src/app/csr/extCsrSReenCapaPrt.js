...
///<xtmlink path="./extCsrSReenCapaPrt.xtm"/>

var extCsrSReenCapaPrt = function() {
	 ...
	 * @param 
	 * @return void
	 * @author Choi In Seong 2016. 5. 25
	 */
...
//		ExtGroup.initSearchBox("grpSearch",["grp_rptCsrShreg"]);
		...
		util.Submit.send(app, "subOnLoad", function(pbSuccess){
			if(pbSuccess){
				...
				// 조회조건 초기화
				util.Control.redraw(app, ["cbbYearRcd", "cbbSmtRcd", "cbbCapIoDivRcd", "dipKeyDate"]);
	...
	moPage.onClick_BtnSearch = function() {
		// 조회조건 필수 체크
		if(!util.validate(app, ["ipbDeptNm", "cbbYearRcd","cbbSmtRcd", "cbbCapIoDivRcd", "dipKeyDate"])){
			return false;
		}
	...
	function doList(poCallBackFunc) {
		...
		util.Submit.send(app, "subList",  function(pbSuccess) {
			if(pbSuccess){
				...
				var vsCapIoDivRcd = util.SelectCtl.getValue(app, "cbbCapIoDivRcd", util.SelectCtl.getSelectedIndex(app, "cbbCapIoDivRcd"));
				...
				var voParam = {
							...
							p_strBefStDate            : util.DataMap.getValue(app, "dmResList", "strBefStDt"),
							p_strBefEndDate         : util.DataMap.getValue(app, "dmResList", "strBefEndDt"),
				 ...
				 var voOzFormParam = {
						  TITLE :  vsSchYearNm + " " + vsSmtNm.substr(0,3) +  " " + vsCapIoDivNm +  " 재입학 여석현황"  //리포트타이틀
						, SUB_TITLE : "" //리포트서브타이틀		
						, FORM_TYPE : "flash"
					};
					
				if(vsCapIoDivRcd == "CT51301") {
					util.Report.open(app, "hojReport", "extCsrSReenCapaPrt", voOzFormParam, voParam);					
				} else if(vsCapIoDivRcd == "BACH")  {
					util.Report.open(app, "hojReport", "extCsrSReenBachCapaPrt", voOzFormParam, voParam);
				} else {
					util.Report.open(app, "hojReport", "extCsrSReenOutCapaPrt", voOzFormParam, voParam);
				}
			}
