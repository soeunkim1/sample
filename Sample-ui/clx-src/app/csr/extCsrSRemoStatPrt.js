...
///<xtmlink path="./extCsrSRemoStatPrt.xtm"/>

var extCsrSRemoStatPrt = function() {
	 ...
	 * @param 
	 * @return void
	 * @author Choi In Seong 2016. 5. 23
	 */
...
//		ExtGroup.initSearchBox("grpSearch",["grp_rptCsrShreg"]);
		...
		util.Submit.send(app, "subOnLoad", function(pbSuccess){
			if(pbSuccess){
				...
				// 조회조건 초기화
				util.Control.redraw(app, ["ipbStDt", "ipbEndDt", "cbbSchYearRcd"]);
		...
		util.Submit.send(app, "subList",  function(pbSuccess) {
			if(pbSuccess){
				...
				if(util.SelectCtl.getSelectedIndex(app, "cbbSchYearRcd") != 0){
					vsSchTitle = vsSchTitle + "    입학년도 : " + vsEntrSchYearNm;
				}
				...
				var voOzFormParam = {
						  TITLE :  "반별 제적생 현황 "//리포트타이틀
						, SUB_TITLE : "" //리포트서브타이틀		
						, FORM_TYPE : "flash"
					};	
						
				util.Report.open(app, "hojReport", "extCsrSRemoStatPrt", voOzFormParam, voParam);
