...
/// <xtmlink path="./extCsrSRetuPlanCallListPrt.xtm"/>


var extCsrSRetuPlanCallListPrt = function() {
	var moPage = new Page();
	...
	moPage.onModelConstructDone_ExtCsrSRetuPlanCallListPrt = function() {
		// 조회조건 초기 설정
...
//		ExtGroup.initSearchBox("grpSearch",["grp_rptCsrShreg"]);
		...
		util.Submit.send(app, "subOnLoad", function(pbSuccess){
			if(pbSuccess){
				...
				util.DataMap.setValue(app, "dmReqList", "strPrtGbn","CALL_INFO");
				util.DataMap.setValue(app, "dmReqList", "strPrtObjDivRcd", "CC009OG");
				
				// 조회조건 초기화
				util.Control.redraw(app, ["cbbYearRcd", "cbbSmtRcd",  "cbbNight","rdbPrtGbn"]);
	...
	moPage.onClick_BtnSearch = function() {
		// 조회조건 필수 체크
		if(!util.validate(app, ["ipbDeptNm", "cbbYearRcd","cbbSmtRcd","rdbPrtGbn"])){
			return false;
		}
	...
	function doList(poCallBackFunc) {
		var vsPrtGbn		= util.Control.getValue(app, "rdbPrtGbn");//출력용도
		var vsPrtGbnNm	=util.SelectCtl.getLabelByValue(app, "rdbPrtGbn",vsPrtGbn);//출력용도 라벨명
		
		var vsOdi = "";
				
		if(vsPrtGbn =="CALL_INFO")
		{
			vsOdi = "extCsrSRetuPlanCallListPrt";
		}else{
			
			vsOdi = "extCsrSRetuPlanClpNoListPrt";
		}
		// submission call 
		//strCommand: list 
		util.Submit.send(app, "subList",  function(pbSuccess) {
			if(pbSuccess){
				...
				var vsNightNm = util.SelectCtl.getLabel(app, "cbbNight", util.SelectCtl.getSelectedIndex(app, "cbbNight"));
				var vsProcRsltNm = util.Control.getValue(app, "ipbProcRslt");
				...
				if(!(ValueUtil.isNull(vsProcRsltNm)||vsProcRsltNm=="")){						
					if (vsSchTitle != "" ) {
						vsSchTitle = vsSchTitle + "     ";
					}
					vsSchTitle = vsSchTitle + "학년 : " + vsProcRsltNm;
				}
				
				if(util.SelectCtl.getSelectedIndex(app, "cbbNight") != 0 ){						
					if (vsSchTitle != "" ) {
						vsSchTitle = vsSchTitle + "     ";
					}
					vsSchTitle = vsSchTitle + "주야 : " + vsNightNm;
				}
				...
				var voParam = {
							...
							p_strCutDate              : util.DataMap.getValue(app, "dmResOnLoad", "strCutDate"),
							...
							p_strNight                   : util.DataMap.getValue(app, "dmReqList", "strNight"),
							p_strPrtObjDivRcd        : "CC009OG",
							...
							p_strSaNm     			 : vsSaNm,
							p_strRtnRsnNm     		 : "",
							p_strNightNm     		 : vsNightNm,
							p_strProcRsltNm     	 : vsProcRsltNm,
							p_strLanDivRcd           : msDefaultLocale
						}	
				
				var vsTitle = vsSchYearNm + " " + vsSmtNm.substr(0,3) + " 복학예정자 "+vsPrtGbnNm;  
				...
				util.Report.open(app, "hojReport",vsOdi, voOzFormParam, voParam);
