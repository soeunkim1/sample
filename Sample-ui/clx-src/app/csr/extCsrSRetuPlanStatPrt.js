...
///<xtmlink path="./extCsrSRetuPlanStatPrt.xtm"/>

var extCsrSRetuPlanStatPrt = function() {
...
//		ExtGroup.initSearchBox("grpSearch",["grp_rptCsrShreg"]);
		...
		util.Submit.send(app, "subOnLoad", function(pbSuccess){
			if(pbSuccess){
				...
				// 조회조건 초기화
				util.Control.redraw(app, ["cbbYearRcd", "cbbSmtRcd"]);
	...
	function doList(poCallBackFunc) {
		...
		util.Submit.send(app, "subList",  function(pbSuccess) {
			if(pbSuccess){
				...
				var vsTitle = vsSchYearNm + " " + vsSmtNm.substr(0,3) + " 복학예정자현황";  
				...
				util.Report.open(app, "hojReport", "extCsrSRetuPlanStatPrt", voOzFormParam, voParam);
