...
///<xtmlink path="./extCsrSShregRemoListPrt.xtm"/>

var extCsrSShregRemoListPrt = function() {
	 ...
	 * @param 
	 * @return void
	 * @author Choi In Seong 2016. 9. 29
	 */
...
//		ExtGroup.initSearchBox("grpSearch",["grp_rptCsrShreg"]);
		...
		util.Submit.send(app, "subOnLoad", function(pbSuccess){
			if(pbSuccess){
				...
				// 조회조건 초기화
				util.Control.redraw(app, ["ipbStDt", "ipbEndDt", "cbbAltRsnRcd","cbbObjDivRcd"]);
	...
	function doList(poCallBackFunc) {
		...
		util.Submit.send(app, "subList",  function(pbSuccess) {
			if(pbSuccess){
				...
				var vsAltRsnRcd = util.SelectCtl.getLabel(app, "cbbAltRsnRcd", util.SelectCtl.getSelectedIndex(app, "cbbAltRsnRcd"));
				...
				if(util.SelectCtl.getSelectedIndex(app, "cbbAltRsnRcd") != 0){
					vsSchTitle = vsSchTitle + "    제적사유 : " + vsAltRsnRcd;
				}
				...
				var voOzFormParam = {
						  TITLE :  "제  적  자  명  단"//리포트타이틀
						, SUB_TITLE : "" //리포트서브타이틀		
						, FORM_TYPE : "flash"
					};	
						
				util.Report.open(app, "hojReport", "extCsrSShregRemoListPrt", voOzFormParam, voParam);
